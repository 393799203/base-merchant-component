...
const valueType = (props, propName, componentName) => {
    ...
    if (props.labelInValue) {
        ...
        if (error) {
            return new Error(
                ...
                `when \`labelInValue\` is \`true\`, \`${propName}\` should in shape of \`{ value: string, label?: string }\`.`
            );
        }
        ...
        if (error) {
            return new Error(
                `Invalid prop \`${propName}\` supplied to \`${componentName}\`, ` +
                `when \`treeCheckable\` and \`treeCheckStrictly\` are \`true\`, \`${propName}\` should in shape of \`{ value: string, label?: string }\`.`
            );
        }
    } else if (props.multiple && props[propName] === '') {
        return new Error(
        `Invalid prop \`${propName}\` of type \`string\` supplied to \`${componentName}\`, expected \`array\` when \`multiple\` is \`true\`.`
        );
    } else {
        const validate = PropTypes.oneOfType([
