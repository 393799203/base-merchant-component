...
const loopTreeData = (data, lv) => {
    ...
    return data.map((item, index) => {
        ...
        if (children && children.length) {
            ret = (<TreeNode {...props}>{loopTreeData(children, pos)}</TreeNode>);
        } else {
            ret = (<TreeNode {...props} isLeaf={isLeaf} />);
        }
        return ret;
    });
};
...
class Select extends Component {
    ...
    // save parsed treeData, for performance (treeData may be very big)
        ...
        this.cacheTreeNodesStates = this.cacheTreeNodesStates !== 'no' &&
                                    this.savedValue &&
                                    nextProps.value === this.savedValue;
        if (this.props.treeData !== nextProps.treeData ||
          this.props.children !== nextProps.children) {
          // refresh this._treeNodesStates cache
            this.treeNodesStates = getTreeNodesStates(
    ...
    componentWillUpdate (nextProps) {
        if (this.savedValue && nextProps.value &&
            nextProps.value !== this.savedValue &&
            nextProps.value === this.props.value) {
            this.cacheTreeNodesStates = false;
  ...
  // combobox ignore
    ...
    onSelect = (selectedKeys, info) => {
        ...
        if (checkEvt) {
            ...
            this.checkedNodes = info.checkedNodesPositions;
            const tree = this.trigger.popupEle;
            this.treeNodesStates = tree.checkKeys;
        } else {
    ...
    getValue ($props, val, ini) {
        let value = val;
        const init = ini || true;
    // if inputValue existing, tree is checkStrictly
        const $strict = init === '__strict' ||
            (init && ((this.state && this.state.inputValue) ||
            this.props.inputValue !== $props.inputValue));
        if ($props.treeCheckable &&
        ($props.treeCheckStrictly || $strict)) {
        ...
        const treeCheckableBool = !!$props.treeCheckable;
        if (!treeCheckableBool || ($props.treeCheckable &&
        ($props.treeCheckStrictly || $strict))) {
            return value;
        }
        let checkedTreeNodes;
        if (this.cachetreeData && this.cacheTreeNodesStates && this.checkedNodes &&
        this.state && !this.state.inputValue) {
            this.checkedTreeNodes = checkedTreeNodes = this.checkedNodes;
        } else {
        /**
        ...
        // getTreeNodesStates is not effective.
            ...
            this.checkedTreeNodes = checkedTreeNodes = this.treeNodesStates.checkedNodes;
        }
        const mapLabVal = arr => arr.map((itemObj) => {
            return {
                value: getValuePropValue(itemObj.node),
                label: getPropValue(itemObj.node, $props.treeNodeLabelProp)
            };
        });
        const props = this.props;
        let checkedValues = [];
    ...
    getDeselectedValue (selectedValue) {
        ...
        this.checkedTreeNodes = this.checkedNodes = newCkTns;
    ...
    removeSelected (selectedVal) {
        ...
        if (props.treeCheckable) {
            if (this.checkedTreeNodes && this.checkedTreeNodes.length) {
                this.checkedTreeNodes = this.checkedNodes = this.checkedTreeNodes.filter((item) => {
    ...
    fireChange (value, extraInfo) {
        ...
        if (vals.length !== sv.length || !vals.every((val, index) => sv[index] === val)) {
            ...
            if (props.treeCheckable && this.state.inputValue) {
                const $vls = [...this.state.value];
                if (ex.checked) {
                    value.forEach((i) => {
                        if ($vls.every(ii => ii.value !== i.value)) {
                            $vls.push({ ...i });
                        }
                    });
                } else {
                    let index;
                    const includeVal = $vls.some((i, ind) => {
                ...
                vls = $vls;
                if (!this.isLabelInValue()) {
                    labs = $vls.map(v => v.label);
                    vls = $vls.map(v => v.value);
                }
            }
            this.savedValue = isMultipleOrTags(props) ? vls : vls[0];
            props.onChange(this.savedValue, labs, ex);
            if (!('value' in props)) {
                this.cacheTreeNodesStates = false;
                this.setState({
                    value: this.getValue(props, toArray(this.savedValue).map((v, i) => {
                        return {
                            value: v,
                            label: labs[i]
                        };
    ...
    renderTopControlNode () {
        ...
        const props = this.props;
        const { prefixCls, maxTagTextLength } = props;
                ...
                return (
                    ...
                    >
                        <span
                            ...
                            onClick={() => { this.removeSelected(singleValue.value); }}
                        />
