...
var UploadImage = React.createClass({
    ...
    render: function() {
        ...
        return (
            <form action={this.props.uploadUrl || SINGEL_UPLOAD_URL} method="post" encType="multipart/form-data" target="J_UploadImageFrame">
    ...
    uploadImgs(files, function(resultArr, i){
        if(resultArr && resultArr.length){
            var mapArr = function(){
                _.map(resultArr, function(item, i){
                    if(item.status.code == 1001){
                        // 如果返回的是绝对路径，就不再添加cdn地址
                        var path = item.result.path ;
                        /^http.+/.test(path) ? paths.push(path) : paths.push(CDN_URL + path);
                    }else{
                        throw new Error( item.status.msg );
                    }
                });
            };
            ...
            try {
                ...
                console.log(e);
                self.props.onFailed(`上传失败:${e.message}`);
            }
        }else{
...
var uploadImgs = function(files, callback , uploadUrl) {
    ...
    var self = this,
        _url = uploadUrl || MULTIPLE_UPLOAD_URL,
        i = 0,
